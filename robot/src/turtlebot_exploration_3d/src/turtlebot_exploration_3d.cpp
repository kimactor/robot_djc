...
int main(int argc, char **argv) {
    ...
    ros::Time now_marker = ros::Time(0);
    ...
    for(int i =0; i < 6; i++){
        ...
        ros::Time start_turn = ros::Time(0);
...
// Evaluate MI for every candidate view points
        ...
        begin_mi_eva_secs = ros::Time(0).toSec();
        ...
        for (int bay_itr = 0; bay_itr < num_of_bay; bay_itr++) {
            ...
            train_time = ros::Time(0).toSec();
            g.train(gp_train_x, gp_train_label);
            train_time = ros::Time(0).toSec() - train_time;

            test_time = ros::Time(0).toSec();
            g.test(gp_test_x, gp_mean_MI, gp_var_MI);
            test_time = ros::Time(0).toSec() - test_time;
        ...
        end_mi_eva_secs = ros::Time(0).toSec();
        ...
        for (int i = 0; i < candidates.size(); i++)
        {
            ...
            CandidatesMarker_array.markers[i].header.stamp = ros::Time(0);
        ...
        while (!arrived) {
            ...
            if(arrived)
            {
                ...
                msg_octomap.header.stamp = ros::Time(0);
                ...
                // explo_log_file << "DA Step ," << robot_step_counter << ", Current Entropy ," << countFreeVolume(cur_tree) << ", time, " << ros::Time(0).toSec() << endl;
